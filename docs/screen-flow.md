...
| 14 | `breaktime` | 「休憩時間中はイベントPtを…」 | **必ず `eventtop` より先。** 背後の「イベント楽曲」が 0.943 で当たるため |
| 15 | `title` | **右下の MENU ボタン**（しきい値 0.92） | **`formation` より後ろ。** 編成画面の MENU が 0.889 で当たるため |
| 16 | `notice` | 起動時の注意書きの「TAP SCREEN」 | |
| 17 | `news` | ヘッダ「お知らせ」 | シアン系ヘッダを持つので `cardx` より先に |
| 18 | `eventtop` | 「イベント楽曲」ボタン | |
| 19 | `home` | EVENT ラベル帯（ベージュ側） | 同上 |
| 20 | `expresult` | 「獲得キャラEXP」 | カードの緑ヘッダを `cardx` が拾うので先に |
| 21 | `cardx` | シアン→緑のヘッダ帯を**色検出** | テンプレ非依存（端末非依存） |
| 22 | `closex` | ポップアップ右上の× | |
| 23 | `eventresult` | EVENT RESULT 見出し | |
| 24 | `result` | per-song「Result」 | 閾値 0.55 と低いが、誤検出しても中央タップのみで安全 |
| 25 | `songselect` | NEXT ボタン | |
| 26 | `friendselect` | フレンド選択 | |
| 27 | `formation` | START ボタン（再判定） | |
| 28 | `menu` | **どれにも当てはまらない** | **クリックせず待つ**。25秒で安全停止 |
