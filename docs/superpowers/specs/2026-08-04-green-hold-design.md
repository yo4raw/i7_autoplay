...
**白率（`min(R,G,B) > 170` の画素割合）が第二の判別軸になる。** 判定文字は白い字を
含むので 0.37、実ノーツは 0.00。これを上限で縛れば 0.65 でも弾ける。二重に守る。

```python
GREEN_APPROACH_FRAC = 0.70   # 緑の検色点（赤の 0.65 とは別。判定文字を避ける）
GREEN_DELTA = 40             # 明るい画素平均で G-R, G-B がこれ以上なら緑（要実機較正）
GREEN_MAX_WHITE = 0.20       # 白率がこれ以上なら判定文字とみなして弾く
...
| 値 | 決め方 | 現状 |
|---|---|---|
| `GREEN_DELTA` | 実機のライブ映像から緑ノーツの RGB を実測 | **未測定。コーパスに緑ノーツが1枚も無い** |
| `TRANSIT_SEC` | approach 点(0.70)から円までの飛行時間。**赤で測れる** — `_approach_red` が True になってから `fired` が立つまでの時間を記録して中央値を取る | 未測定 |
| `HOLD_MAX_SEC` | 実測 7 秒程度（ユーザー）＋余裕で **8 秒** | 現行 2.5 秒から変更 |
...
stateDiagram-v2
    [*] --> 待機
    待機 --> 保持: 緑を見た(GREEN_MEMORY内) かつ fired
    保持 --> 解除予約: 次の緑が approach を通過
...
1. `_approach_green` が判定文字「GOOD」のフレームで **False** を返すこと（回帰の要）
2. 白率の上限が効いていること（`GREEN_MAX_WHITE` を緩めると GOOD を拾うことを示す）
...
| リスク | 対応 |
|---|---|
| **緑ノーツの実物を1枚も持っていない。** `GREEN_DELTA` が机上の値 | 実装の第1ステップを「実機でライブを1回録って緑ノーツの RGB を測る」にする。測るまで実装を進めない |
...
1. **実機でライブを1回録り、緑ノーツの RGB と `TRANSIT_SEC` を実測する**（ここが前提）
