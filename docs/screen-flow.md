...
| 17 | `title` | **右下の MENU ボタン**（しきい値 0.92） | **`formation` より後ろ。** 編成画面の MENU が 0.889 で当たるため |
| 18 | `notice` | 起動時の注意書きの「TAP SCREEN」 | |
| 19 | `news` | ヘッダ「お知らせ」 | シアン系ヘッダを持つので `cardx` より先に |
| 20 | `eventtop` | 「イベント楽曲」ボタン | |
| 21 | `home` | EVENT ラベル帯（ベージュ側） | 同上 |
| 22 | `songselect` | NEXT ボタン | **`cardx` より先。** イベント楽曲リスト上部の薄い水色の区切り線（高さ2px）を `detect_card_x` がシアン帯として拾うため（2026-08-28 実機で `cardx_stuck`） |
| 23 | `expresult` | 「獲得キャラEXP」 | カードの緑ヘッダを `cardx` が拾うので先に |
| 24 | `cardx` | シアン→緑のヘッダ帯を**色検出** | テンプレ非依存（端末非依存） |
| 25 | `closex` | ポップアップ右上の× | |
| 26 | `eventresult` | EVENT RESULT 見出し | |
| 27 | `result` | per-song「Result」 | 閾値 0.55 と低いが、誤検出しても中央タップのみで安全 |
| 28 | `friendselect` | フレンド選択 | |
| 29 | `formation` | START ボタン（再判定） | |
| 30 | `menu` | **どれにも当てはまらない** | **クリックせず待つ**。25秒で安全停止 |
...
| 状態 | 操作 | 補足 |
|---|---|---|
| `songselect` | 対象曲を選択 → EASY タブ → NEXT | 曲テンプレに一致しなければ**曲を変更しない**（安全側）。`--keep-selection` の回は曲も難易度も選び直さず NEXT のみ押す（累計イベント以外の例外運用） |
