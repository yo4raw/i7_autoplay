...
| 項目 | 状態 | コミット / 備考 |
|---|---|---|
| **C-1** ステラ誤消費（既遂） | **修正済み** | きなこパン行の実在確認を必須化。実機で `行確認 score=0.88` を確認 |
| **H-1** 安全停止が rc=0 で空転 | **修正済み** | 停止理由8種＋`EXIT_SAFE_STOP`(42)。supervisor は再起動せず、バックオフと空転打ち切りも追加 |
| **H-2** EXP画面の cardx 誤判定 | **修正済み** | `expresult` 状態を新設。**警告の39%（34/87件）を占めた最多要因** |
| **H-3** 切断ガードの評価タイミング | **修正済み** | H-1 で大部分が解消。残る「runnerレベルの無限ループ」をフラグ連携で解消 |
| **H-4** sentinel/guard の kill 競合 | **修正済み** | プロセスの生殺与奪を `run_until.sh` に一本化 |
| **M-2** ウィンドウ矩形の再取得 | **修正済み** | 2秒間隔で取り直し、寸法変化時は座標系を作り直す |
| 色空間の取り違え（本レビュー外で発覚） | **修正済み** | テンプレはBGR・フレームはRGBで照合していた。スコア 0.729→1.000 |

修正後の実測（19:33 以降、57分間）: **26周クリア・警告0件・PAUSE 0件・supervisor 再起動0回**。
修正前は警告87件（うち cardx 停滞34件）、「完了: 0 回クリア」71件の空転があった。
テストは 52件 → **84件**。

残りは Medium 13件・Low 58件。着手順は本文末尾の「着手順の提案」を参照。
