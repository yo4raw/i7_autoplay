...
2. **難易度は必ず EASY。**
3. **イベント楽曲へはホームの EVENT リボンから入る。** ホームの「LIVE」ボタンから入ると
   通常ライブで、クリアしてもイベント pt が一切付かない。
   > リボンは**2つ並ぶ**（2026-08-02 実測。位置は左下ではなく**右側**）。
   > **ラベル帯がベージュの方**が累計イベント。下段のバナーは数秒で切り替わる
   > カルーセルなので目印に使えない。詳細: [`docs/navigation.md`](docs/navigation.md)
4. **ブースト倍率は必ず 3 倍。**
...
6. **未知の明るい画面ではクリックせず停止する。** 盲目連打はしない（課金ボタンの誤タップ防止）。
   **休憩時間中は周回しない。** イベントトップから楽曲へ進むとき休憩中は確認ダイアログが出る。
   「はい」を押すと**イベント pt がゼロのまま LIFE を消費する**ので、必ず「いいえ」を押して
   停止する（`stop_reason=break_time`）。休憩時間帯は**ゲーム内のユーザー設定で可変**なので、
   時刻から判断してはいけない。
