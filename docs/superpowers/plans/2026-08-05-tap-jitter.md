...
重ね、合計が上限半径を超えたら方向を保って切り詰める。ウィンドウ寸法を
知らない純粋モジュールなので実機なしでテストできる。"
```
...
最大 12px でリング半径 26px より小さいため、フリック方向はほとんど変わらない。
...
判定（spec の「CLI と既定値」表）:
  - MISS 増が誤差範囲（±5%）内            → 既定 ON を維持
  - MISS が有意に増えた                    → JITTER_MAX_R を 0.60 → 0.30 に下げて再測定
...
- **リング内なら判定が変わらないこと**は未確認。Task 3 Step 12 の A/B で確認する
