...
| 22 | `expresult` | 「獲得キャラEXP」 | カードの緑ヘッダを `cardx` が拾うので先に |
| 23 | `cardx` | シアン→緑のヘッダ帯を**色検出** | テンプレ非依存（端末非依存） |
| 24 | `closex` | ポップアップ右上の× | |
| 25 | `eventresult` | EVENT RESULT 見出し | |
| 26 | `result` | per-song「Result」 | 閾値 0.55 と低いが、誤検出しても中央タップのみで安全 |
| 27 | `songselect` | NEXT ボタン | |
